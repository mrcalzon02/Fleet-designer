...
function billForLicensedListing(listing) {
  if (listing.type === 'vessel') return { rawOre: 8, electronics: 5, hullPlate: 9, driveCores: 1 };
  if (listing.type === 'module') {
    if (listing.category === 'propulsion') return { rawOre: 3, electronics: 3, hullPlate: 3, driveCores: 1 };
    if (listing.category === 'power') return { electronics: 4, hullPlate: 2, volatiles: 3 };
    if (listing.category === 'thermal') return { electronics: 2, hullPlate: 2, volatiles: 1 };
    if (listing.category === 'structure') return { rawOre: 4, hullPlate: 4 };
    if (listing.category === 'cargo') return { rawOre: 4, electronics: 1, hullPlate: 4 };
    if (listing.category === 'control') return { electronics: 5, hullPlate: 1 };
    return { rawOre: 4, electronics: 2, hullPlate: 3 };
  }
  return { electronics: 3, hullPlate: 1 };
}

function productionCostForLicensedListing(listing) {
  if (listing.cost) return listing.cost;
  const qualityPressure = (listing.quality ?? 66) / 66;
  const base = listing.type === 'vessel' ? 1600000 : listing.type === 'module' ? 520000 : 240000;
  return Math.round(base * qualityPressure);
}

function salePriceForLicensedListing(listing, cost) {
  if (listing.salePrice) return listing.salePrice;
  const reliabilityPressure = 1 + Math.max(0, (listing.reliability ?? 70) - 55) / 100;
  const base = listing.type === 'vessel' ? 3000000 : listing.type === 'module' ? 880000 : 420000;
...
export function buyLicense(state, listingId) {
  ...
  const quality = listing.quality ?? 66;
  const reliability = listing.reliability ?? 70;
  const cost = productionCostForLicensedListing(listing);
  const salePrice = salePriceForLicensedListing(listing, cost);
  ...
  next.designs.push({
    ...
    type: listing.type,
    moduleCategory: listing.category,
    quality,
    reliability,
    cost,
    salePrice,
    licensePrice: 0,
    bill: billForLicensedListing(listing),
    rights: 'licensed',
    licensedFrom: listing.seller,
    sourceListingId: listing.id,
    marketListed: false,
    description: `Licensed production pattern purchased from ${listing.seller}. ${listing.source === 'rival-generated' ? 'Generated by active rival market pressure.' : 'Purchased from the general license market.'}`,
  });
  listing.purchasedCycle = next.company.cycle;
  listing.license = false;
  next.eventLog.unshift(`Cycle ${next.company.cycle}: Purchased production license for ${listing.designName} from ${listing.seller}. Quality ${quality}, reliability ${reliability}.`);
  return next;
}
